...
#import "PCLocalNotificationService.h"

#import "PCSystemService.h"

@interface PCLocalNotificationService (Category)
+ (NSUInteger)UIUserNotificationAvailableVersion;
+ (NSString *)categoryName;
+ (NSString *)categoryNameWithSubCategory:(NSString *)subCategory;
@end

@implementation PCLocalNotificationService

- (instancetype)init {
    self = [super init];
    if (self) {
        [[PCSystemService sharedService] OSMajorVersionBranching:[self.class UIUserNotificationAvailableVersion] laterBlock:^{
...
@implementation PCLocalNotificationService (Category)
